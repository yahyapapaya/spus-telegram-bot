...
If those repository variables are not set, the workflow now falls back to safe defaults:
- `WEIGHT_CHANGE_THRESHOLD_PCT=0.10`
- `TOP_WEIGHT_MOVERS=15`

### Workflow troubleshooting

- Ensure Actions secrets are set:
  - `TG_BOT_TOKEN`
  - `TG_CHAT_ID`
- The workflow now uses newer GitHub Actions versions (`actions/checkout@v5`, `actions/setup-python@v6`) to avoid Node 20 deprecation risk.
